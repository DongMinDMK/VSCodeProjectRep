...
// 전문가가 사용한 정규표현식은 초보자에가 외계언어와 같은 느낌의 어려운 식들이지만, 문자탐색과 스캔이 있어 강력한 기능을 

// 갖고 있어, 여러분야에 많이 사용되고 있습니다.

// 자바스크립트에서도 많은 다양한 정규표현식의 적용을 지원하고 있습니다.

let a = "While some may view this dept forgiveness as a slap in the face to people who were responsible and paid off their student loans";
...
console.log(`검색 첫번째 결과 텍스트 result[0] : ${result[0]}`); //첫번째 매칭 결과 문자
...
a = "gabbvdrfabgrdsagfrea";

console.log(`대상text : ${a}`);

console.log(`검색패턴 : /ab*/g`);

result = a.match(/ab*/g); // b가 없어도 검색대상 포함
...
result = a.match(/ab+/g); // b가 없으면 검색대상에서 제외
...
// [] : 괄호 안에 검색하고자 하는 글자들을 넣고, 그 포함유무를 판단합니다.

// [abc] : a와b와 c가 대상 문자열 안에 하나라도 포함되었는지 판단합니다.

// [abc] : a 또는 b 또는 c 를 의미합니다.
...
console.log(result); // 매칭결과가 없으면 결과는 null 입니다.
...
// 활용 : 다른글자와 조합해서 아래와 같은 검색으로 확장됩니다.

text = "grezageagfzvrzce";
...
// 숫자를 검색하고 추출하는 경우 [0123456789] -> [0-9] 중 하나

// 문자(소문자) 검색 [abcdefghijklmnopqrstuvwxyz] -> [a-z] 중 하나

// 문자(대문자) 검색 [ABCDEFGHIJKLMNOPQRSTUVWXYZ] -> [A-Z] 중 하나
...
text = "ABCDEFGHIJKLMN0PQR5TUVWXY2";
...
// [\W] : 문자와 숫자가 이난 것과 매치[^0-9a-zA-Z] 와 같은 표현
...
// a.b : a 와 b 사이에 어떤 글자가 들어와도 매칭됩니다.
...
text = "fdfabfda"; // a와 b 사이에 글자가 없는 것은 매칭되지 않습니다.
...
text = "현재 접속중인 외부 아이피는 110.8.6.181 이며, 내부아이피는 192.168.0.44 입니다.";

// 위 text 에서 아이피주소 내부의 숫자들만 골라서 출력하세요.
...
// 각각의 숫자가 아닌 완벽한 아이피 주소를 매칭해서 출력하세요.
